"""Монте-Карло: Герцог 3 ур. + 2 Тёмных паладина спереди, Демонолог сзади — против Чёрного дракона.
...
DRAGON_HP = int(sys.argv[1]) if len(sys.argv) > 1 else 800


def run_one(log: bool = False) -> tuple:
    env = BattleEnv(log_enabled=log)
    dragon0 = make_unit("Чёрный дракон", "red", 2, "ahead")
...
def main():
    ...
    for n in ("Темный паладин", "Демонолог", "Чёрный дракон"):
    ...
    print(f"Средний урон по дракону: {sum(dragon_dmg)/N:.0f} из 800 ({100*sum(dragon_dmg)/N/800:.0f}%)")
    print(f"Лучший бой (макс. урон по дракону): {max(dragon_dmg)} из 800")
