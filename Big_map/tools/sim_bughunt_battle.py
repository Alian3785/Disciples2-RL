...
BLUE управляется случайными валидными действиями (по маске), RED — скриптованным ИИ
среды. После каждого шага проверяются инварианты; расхождения печатаются в отчёте.
...
def run_battle(
    ...
    *,
    ...
    red = build_team("red", red_spec, range(1, 7))
    blue = build_team("blue", blue_spec, range(7, 13))
    ...
    if dump_log or result["anomalies"]:
        out_dir = PROJECT_ROOT / "outputs" / "bughunt"
        out_dir.mkdir(parents=True, exist_ok=True)
        log_path = out_dir / f"battle_seed{seed}.log"
...
def main() -> None:
    ...
    for i in range(args.battles):
        ...
        if "expected_exp" in res and res["winner"] == "blue":
