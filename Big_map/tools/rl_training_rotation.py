...
OBJECTIVES = ("cities", "green_dragon", "blue_dragon")
NEXT_OBJECTIVE = {
    "cities": "green_dragon",
    "green_dragon": "blue_dragon",
    "blue_dragon": "cities",
}
OBJECTIVE_FLAGS = {
    "cities": "",
...
def load_state(path: Path) -> dict[str, Any]:
    ...
    if data.get("next_objective") not in OBJECTIVES:
        raise ValueError(f"state.next_objective must be one of {OBJECTIVES}")
...
def prepare_run(
    *,
    ...
    output_base: Path,
    event_name: str,
    activation_slot: bool,
    manual_objective: str,
    run_token: str,
) -> dict[str, str]:
    is_manual = event_name == "workflow_dispatch"
    should_run = is_manual or bool(state["active"]) or activation_slot
    if not should_run:
        return {
            "should_run": "false",
            "reason": "waiting-for-next-23-17-activation-slot",
        }

    objective = manual_objective if is_manual else str(state["next_objective"])
    seed = int(state["seed"])
    token = _safe_token(run_token)
    prefix = "smoke" if is_manual else f"seed{seed}-{objective}"
    ...
    if is_manual:
        ...
        run_dir = output_base / str(seed) / objective / run_id
    ...
    return {
        ...
        "seed": str(seed),
        "objective": objective,
        "objective_flag": OBJECTIVE_FLAGS[objective],
        "run_id": run_id,
...
def finalize_success(
    *,
    ...
    if seed != state["seed"] or objective != state["next_objective"]:
    ...
    next_objective = NEXT_OBJECTIVE[objective]
    next_seed = seed + 1 if objective == "blue_dragon" else seed
    manifest = {
        "status": "success",
        "run_id": run_id,
        "seed": seed,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prepare.add_argument("--activation-slot", action="store_true")
    prepare.add_argument("--manual-objective", choices=OBJECTIVES, default="cities")
    ...
    complete.add_argument("--objective", choices=OBJECTIVES, required=True)
...
def main() -> int:
    ...
    if args.command == "prepare":
        values = prepare_run(
            ...
            manual_objective=args.manual_objective,
