...
# - Recommends new entrypoints: battle_env.py, train_sb3.py, visualize.py
# - Keeps ability to trigger train or viz from this file
# ============================================================
...
# WandbCallback доступен только если wandb установлен и включен
if WANDB_AVAILABLE:
...
check_env(_BattleEnvCheck(log_enabled=False), warn=True)

"""
Legacy monolith contained training and visualization below.
This file now serves as an orchestrator.
Use:
  python -m train_sb3
  python -m visualize --ckpt <path>
"""
...
def _print_help():
    print("Usage:")
    print("  Train:       python -m train_sb3")
    print("  Visualize:   python -m visualize --ckpt <path_to_zip>")
    print("  Only env:    python -m battle_env  (runs env check)")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("cmd", nargs="?", choices=["train", "viz", "env"], help="What to run")
    parser.add_argument("--ckpt", help="Checkpoint path for viz")
    ...
    if args.cmd == "train":
        ...
        if not args.ckpt:
            ...
            sys.exit(1)
        import visualize
        visualize.play_checkpoint(args.ckpt)
    elif args.cmd == "env":
        os.system(f"{sys.executable} -m battle_env")
    else:
        _print_help()
        sys.exit(0)
    ...
    try:
        run = wandb.init(
            ...
            config={
                ...
                "eval_freq": EVAL_FREQ,
                "obs_dim": 12*45,  # <— обновлено под Точность2
                "types": TYPE_LIST,
...
if USE_WANDB:
    ...
    _callbacks = [wandb_cb, eval_cb, checkpoint_cb]
else:
    _callbacks = [eval_cb, checkpoint_cb]
...
if VISUALIZE_TEST:
    ...
    if IS_NOTEBOOK:
        # в ноутбуке не держим отдельное окно
        plt.close(fig)
    else:
        # в обычном Python включим интерактив и покажем окно
        plt.ion()
        try:
            fig.show()
        except Exception:
